...
app.get("/jokes/:id",(req,res)=>{
 ...
 const joke = jokes.find((x)=> x.id===id);
 res.json(joke);
});
...
app.get("/filter",(req,res)=>{
  ...
  const joke = jokes.filter((x)=> x.jokeType === type);
  res.json(joke);
});

app.post("/jokes",(req,res)=>{
  const jType = req.body.type;
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  const newJoke = {
    ...
    jokeType:req.body.type || existJoke.jokeText
  }
...
app.delete("/jokes/:id",(req,res)=>{
  ...
  const index1 = jokes.findIndex((joke)=>joke.id===ID1);
  if (index1 >-1){
    jokes.splice(index1 ,1);
    res.sendStatus(200);
  }
  else{
